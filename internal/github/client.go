...
// operations return immediately with a nil RepoData.
...
func (c *Client) Fetch(ctx context.Context, owner, repo string, opts FetchOptions) *RepoData {
	if !c.available {
		return nil
	}
	...
	data := collectRepoData(ctx, c, owner, repo, opts.FetchSecurity)
...
func (c *Client) SetDescription(ctx context.Context, owner, repo, description string) error {
	if !c.available {
		return fmt.Errorf("github: no token available")
	}
