...
// collectRepoData fetches repository metadata from the GitHub API.
//
...
func collectRepoData(ctx context.Context, c *Client, owner, repo string, fetchSecurity bool) *RepoData {
	data := NewRepoData(owner, repo)
	...
	if err == nil {
		...
		data.OpenIssues = data.OpenIssues - prCount
...
func populateFromRepo(data *RepoData, r *gogithub.Repository) {
...
func CollectSecurityAlerts(ctx context.Context, c *Client, data *RepoData) {
