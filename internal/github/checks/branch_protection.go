...
// BranchProtectionMissing detects repos where the default branch
// has no branch protection rules configured on GitHub.
	...
	return BranchProtectionMissing{
		githubCheck: githubCheck{
			Describer: models.NewDescriber(
				...
				"detects repos with no branch protection on the default branch",
			),
		},
	}
}
...
func (c BranchProtectionMissing) evaluate(data *models.PlatformInfo) (iter.Seq[models.Alert], error) {
	...
	// Only suggest the action if the user has admin access.
	var suggestions []models.ActionSuggestion
	if data.HasAdminAccess {
		suggestions = []models.ActionSuggestion{
			repoSuggestion("enable-branch-protection", simpleSubject(data.FullName)),
		}
	} else {
		detail += " (admin access required to enable protection)"
	}
