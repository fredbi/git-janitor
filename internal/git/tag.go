...
var (
	...
	semverWithV = regexp.MustCompile(`^v(\d+)\.(\d+)\.(\d+)(?:-([a-zA-Z0-9.]+))?(?:\+[a-zA-Z0-9.]+)?$`) //nolint:gochecknoglobals

	// semverWithoutV matches "1.2.3" or "1.2.3-pre.1".
	semverWithoutV = regexp.MustCompile(`^(\d+)\.(\d+)\.(\d+)(?:-([a-zA-Z0-9.]+))?(?:\+[a-zA-Z0-9.]+)?$`) //nolint:gochecknoglobals
)
...
func parseTags(output string) []Tag {
	...
	for scanner.Scan() {
		...
		// Parse date.
		t.Date, _ = time.Parse(time.RFC3339, strings.TrimSpace(dateStr)) //nolint:errcheck // best-effort
...
func parseSemver(t *Tag) {
	...
	//nolint:errcheck // regex guarantees digits
	fmt.Sscanf(matches[0], "%d", &t.SemverMajor)
	fmt.Sscanf(matches[1], "%d", &t.SemverMinor)
	fmt.Sscanf(matches[2], "%d", &t.SemverPatch)
