...
func (c RemoteNoOrigin) Evaluate(ctx context.Context) (iter.Seq[models.Alert], error) {
...
func (c RemoteNoOrigin) evaluate(info *backend.RepoInfo) (iter.Seq[models.Alert], error) {
	...
	origin := backend.FindRemote(info.Remotes, backend.RemoteOrigin)
	if origin != nil {
	...
	if len(info.Remotes) == 1 {
		return singleAlert(models.Alert{
			...
			Summary:   fmt.Sprintf("remote %q should be named %q", info.Remotes[0].Name, backend.RemoteOrigin),
			Detail:    fmt.Sprintf("single remote %q exists but is not named %q", info.Remotes[0].Name, backend.RemoteOrigin),
			Suggestions: []models.ActionSuggestion{{
				ActionName:  "rename-remote",
				SubjectKind: models.SubjectRemote,
				Subjects:    simpleSubject(info.Remotes[0].Name, backend.RemoteOrigin),
			}},
		}), nil
	}
	...
	return singleAlert(models.Alert{
		...
		Detail:    fmt.Sprintf("%d remotes exist but none is named %q", len(info.Remotes), backend.RemoteOrigin),
	}), nil
}
...
func (c RemoteMisnamedUpstream) Evaluate(ctx context.Context) (iter.Seq[models.Alert], error) {
	info, err := repoInfoCtx(ctx)
	if err != nil {
		return nil, err
	}
...
func (c RemoteMisnamedUpstream) evaluate(info *backend.RepoInfo) (iter.Seq[models.Alert], error) {
	...
	if backend.FindRemote(info.Remotes, backend.RemoteUpstream) != nil {
		return noAlert(c.Name())
	}

	// Find the distinct remote (the one that's not origin).
	misnamed, found := backend.HasDistinctRemote(info.Remotes)
	if !found {
		return noAlert(c.Name())
	}
	...
	return singleAlert(models.Alert{
		CheckName: c.Name(),
		Severity:  models.SeverityMedium,
		Summary:   fmt.Sprintf("fork remote %q should be named %q", misnamed, backend.RemoteUpstream),
		Detail:    fmt.Sprintf("remote %q has a different URL from origin — likely a fork source that should be named %q", misnamed, backend.RemoteUpstream),
		Suggestions: []models.ActionSuggestion{{
			ActionName:  "rename-remote",
			SubjectKind: models.SubjectRemote,
			Subjects:    simpleSubject(misnamed, backend.RemoteUpstream),
		}},
	}), nil
}
